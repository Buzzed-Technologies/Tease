...
export async function registerUser(name, phone, password, persona) {
  try {
    ...
    console.log('Supabase client initialized with URL:', supabase?.supabaseUrl ? 'Valid URL' : 'Missing URL');
    
    // Check if phone number already exists
    console.log('Checking if phone number already exists:', phone);
    ...
    if (existingUserError && existingUserError.code !== 'PGRST116') {
      ...
      return { success: false, error: `Database error: ${existingUserError.message}` };
    }
    ...
    console.log('User registered successfully:', data ? 'Data received' : 'No data');
